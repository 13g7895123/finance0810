export const useLeads = () => {
  ...
  const convertToCase = async (lead, payload) => {
    // 若 lead 有綁定 customer，直接送件；否則先嘗試用 phone/email/line 找 customer（此處簡化為傳入 customer_id）
    if (!lead.customer_id) throw new Error('Lead 尚未綁定客戶，請先綁定或建立客戶')
    return await post(`/customers/${lead.customer_id}/cases`, payload)
  }
  ...
  return { list, getOne, updateOne, removeOne, convertToCase }
}
